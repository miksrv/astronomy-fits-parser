...
import numpy as np
from app.fixindex import fixindex
...
def starphot(hw, inset, starpoints, etime, gain, name):
    ...
    if any(x < 0 for x in backsub):
        print('[ ' + colored('ERROR', 'red'),
              '] FITS contains a star with negative background-subtracted value - no magnitude calculated.')

    # Calculate mag errors
    magerr = [1 / np.sqrt(x) if x > 0 else 0 for x in backsub]

    # Convert to flux by dividing by exposure time
    flux = [x / etime for x in backsub]

    # Convert to magnitude
    mags = [-2.5 * np.log10(a) + 20 if a > 0 else 0 for a in flux]
